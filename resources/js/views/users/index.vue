...
<!--      </el-select>-->
    ...
    <el-table v-loading="loading" :data="list" border fit highlight-current-row style="width: 100%">
      ...
      <el-table-column align="center" label="Register Date">
        <template slot-scope="scope">
      ...
      <el-table-column align="center" label="Date">
        <template slot-scope="scope">
          <span>{{ scope.row.last_app_date }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="Last Visit">
        <template slot-scope="scope">
          <span>{{ scope.row.last_visit }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="Total Appts">
        <template slot-scope="{row}">
          <el-button type="info" size="small" @click="showDialog(row)">
            {{ row.total_appts }}
          </el-button>
        </template>
      </el-table-column>

      <el-table-column align="center" label="Canceled Appts">
        <template slot-scope="scope">
          <span>{{ scope.row.canceled_appts }}</span>
        </template>
      </el-table-column>

      <el-table-column align="center" label="Edited Appts">
        <template slot-scope="scope">
          <span>{{ scope.row.edited_appts }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column align="center" label="Actions" width="100">
        <template slot-scope="scope">
          ...
          <!--            Delete-->
          <!--          </el-button>-->
          <router-link v-if="!scope.row.roles.includes('admin')" :to="'#'">
            <el-button v-permission="['manage user']" type="info" size="small" icon="el-icon-edit" disabled>
              Block
            </el-button>
          </router-link>
        </template>
            ...
            <el-form :model="currentUser" label-width="80px" label-position="top">
              <el-form-item label="Permissions">
                <el-tree ref="otherPermissions" :data="normalizedOtherPermissions"
                         :default-checked-keys="permissionKeys(userOtherPermissions)" :props="permissionProps"
                         show-checkbox node-key="id" class="permission-tree"/>
              </el-form-item>
            </el-form>
          </div>
    ...
    methods: {
      ...
      async getList() {
        ...
        this.list.forEach((element, index) => {
          element['total_appts'] = Math.floor(Math.random() * 10);
